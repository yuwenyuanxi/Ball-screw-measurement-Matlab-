...
function [hough_space,hough_circle_func,para] = hough_circle_fun(BW,step_r,step_angle,r_min,r_max,p)
...
hough_circle_func = false(m,n);
%hough_circle_func = zeros(m,n);
%通过位置求半径和圆心。
for i=1:ecount
    for k=1:length
        ...
        if((rows(i)-par1)^2+(cols(i)-par2)^2<(r_min+(par3-1)*step_r)^2+5&&...
            ...
            hough_circle_func(rows(i),cols(i)) = true;   %检测的圆
        end
    end
end
...
for k=1:length
    ...
    hough_circle_func(par1,par2)= true; %这时得到好多圆心和半径，不同的圆的圆心处聚集好多点，这是因为所给的圆不是标准的圆
